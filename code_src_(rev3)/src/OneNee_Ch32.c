...
// OneNee 
// Porting to CH32V003 by Carmax91 rev3 heavly based on kalymos PsNee_v7/v8 (https://github.com/kalymos/PsNee)
//
...
// Rev3: On the Injection function updated WFCK modulation for 7.3/14.6 kHz compatibility
...
void NTSC_fix()
{
  ...
  while (!(GPIOD->INDR >> (5 & 0xf) & 1)) //(bios_A18) == 0)
  {
  ...
  GPIOD->CFGLR |= (3 | 0) << (4 * 6);
  //------------------------------------------------------------------
  GPIOD->BSHR = (1 << 16 + 6); // PC6 -> Bios_D2 = output. Drags line low now
